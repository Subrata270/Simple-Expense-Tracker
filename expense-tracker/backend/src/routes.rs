...
pub fn init(cfg: &mut web::ServiceConfig) {
    cfg
        ...
        .service(web::resource("/expenses/summary")
...
async fn get_expenses(pool: web::Data<SqlitePool>) -> impl Responder {
    let rows = sqlx::query_as!(
        Expense,
        r#"SELECT id, amount, description, category, expense_date, created_at, updated_at FROM expenses ORDER BY expense_date DESC"#
    )
...
async fn get_expense(pool: web::Data<SqlitePool>, id: web::Path<String>) -> impl Responder {
    let row = sqlx::query_as!(
        Expense,
        r#"SELECT id, amount, description, category, expense_date, created_at, updated_at FROM expenses WHERE id = ?"#,
        id.into_inner()
    )
    ...
    match row {
        ...
        Ok(None) => HttpResponse::NotFound().json(json!({ "error": "Expense not found" })),
        Err(e) => HttpResponse::InternalServerError().json(json!({ "error": format!("DB error: {}", e) })),
    }
}
...
async fn update_expense(
    ...
    .await;
    match result {
        Ok(r) if r.rows_affected() > 0 => HttpResponse::Ok().json(json!({ "status": "Updated" })),
        Ok(_) => HttpResponse::NotFound().json(json!({ "error": "Expense not found" })),
        Err(e) => HttpResponse::InternalServerError().json(json!({ "error": format!("DB error: {}", e) })),
    }
}
